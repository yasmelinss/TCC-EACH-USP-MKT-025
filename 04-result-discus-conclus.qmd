...
- AQUI FICARIA LEGAL COLOCAR A IMAGEM DE DENSIDADE E MAPA DE CALOR
...
Para caracterizar o papel de cada ator na rede de interações analisada, foram calculadas quatro métricas derivadas da estrutura direcional do grafo de likes: grau de entrada (in-degree), grau de saída (out-degree), saldo de interações e spread, adotando-se a convenção de que o in-degree representa likes dados (influência recebida) e o out-degree representa likes recebidos (influência dada).

O grau de entrada (in-degree) corresponde ao número total de likes dados por cada ator. Em termos de teoria de redes, essa métrica contabiliza as arestas que chegam ao nó, consideradas aqui como ações iniciadas pelo ator. Assim, um in-degree elevado indica perfis altamente ativos na plataforma, que engajam frequentemente com conteúdos de outros atores.

O grau de saída (out-degree), por sua vez, representa o total de likes recebidos por um ator, isto é, as arestas que partem do nó. Dessa forma, o out-degree expressa o nível de visibilidade ou popularidade de cada perfil, refletindo o quanto ele atrai atenção e engajamento da comunidade.
...
top_ativos |>
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "atores que mais engajam - dia 07/09/2024"
  ) |> kableExtra::kable_styling(latex_options = "hold_position")
```
...
A análise das métricas de interação do dia 37 revela padrões claros de comportamento na rede. Os valores de in_degree, definidos como o total de likes dados por cada ator, evidenciam perfis extremamente ativos. Os destaques são ilhadaslendas.com (10.706 likes), LTA_SUL (10.521 likes) e pain.gg (3.881 likes), indicando contas que interagem intensamente com outros usuários.

Em contraste, o out_degree, entendido como o volume de likes recebidos, apresenta valores significativamente menores. Apenas alguns perfis receberam quantidades expressivas de likes, como pereirawesley.bsky.social (56 likes) e pain.gg (26 likes). Isso sugere uma estrutura de engajamento assimétrica, em que a maioria dos atores distribui muito mais interações do que recebe.

O spread, que reflete a parte recíproca da atividade (interações tanto de ida quanto de volta), confirma essa assimetria. Embora a maior parte dos atores apresente valores baixos, alguns perfis, como pereirawesley.bsky.social, mantêm um nível de reciprocidade mais elevado, indicando trocas mais equilibradas de engajamento.
