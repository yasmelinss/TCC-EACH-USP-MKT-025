...
'at://did:plc:g4efut5lyfkz6wxoejsuo2dw/app.bsky.feed.post/3m2i7h6uhx222' |> 
bs_uri_to_url()

'at://did:plc:kwssn4xujlrl4ybhzm5vt5mg/app.bsky.feed.post/3l5xjaj66js24' |> 
  bs_uri_to_url()

'https://bsky.app/profile/pupn1k.bsky.social/post/3m2kh4cgtk22q' |> 
  bs_url_to_uri(auth = auth)

'https://bsky.app/profile/yasmelinss.bsky.social' |> 
  bs_url_to_uri(auth = auth)
```
...
did <- 
  'at://did:plc:vljand5f4megiw7rj4mpmhaw/app.bsky.feed.post/3l6br5zrsda2r' |> 
  str_extract("did:[^/]+")
...
filtragem <- 
    tibble(
      ...
      qnt_seguidores_em_comum = 
        perfil$profiles[[1]]$viewer$knownFollowers$count
    )

formataJSON(filtragem)
```
...
while (i <= length(qnt_seguidores_em_comum)) {
  filtragem2 <- 
    tibble(
      perfis_comum_did =
        perfil$profiles[[1]]$viewer$knownFollowers$followers[[i]]$did, 
      perfis_comum_handle =
        perfil$profiles[[1]]$viewer$knownFollowers$followers[[i]]$handle, 
      perfis_comum_displayName =
        perfil$profiles[[1]]$viewer$knownFollowers$followers[[i]]$displayName
    )
  i = i + 1
}

formataJSON(filtragem2)
```
...
'https://bsky.app/profile/yasmelins.bsky.social/post/3l6br5zrsda2r' |> 
  bs_url_to_uri(auth = auth)
```
...
likes <- 
  bs_get_likes(
    actor = 'ennoe.bsky.social', 
      # diz de quem você quer pegar os likes, e espera um uri, não um handle
    limit = 3,
    auth = auth
  )
