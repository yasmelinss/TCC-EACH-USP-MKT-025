...
Então, para pegar apenas os dados que interessam (id, usuário, texto, data), você precisa desaninhar essas listas.

nrow(posts)

posts é um tibble/data.frame com várias linhas (cada linha = 1 post do Bluesky).

nrow(posts) retorna o número de posts raspados.

1:`nrow(posts)` gera um vetor de índices: se tiver 2 posts, vira c(1, 2).

2. `{map_dfr}`(...)

Vem do pacote purrr (já incluso no tidyverse).

Ele aplica uma função para cada elemento de um vetor.

O sufixo _dfr significa: mapear e colar o resultado como linhas de um data.frame/tibble.
...
#| label: mostrando-resumo-post-id-usuario-conteudo-data-curtidas-reposts
#| eval: true
...
cat("aqui estão os dados filtrados da segunda forma\n")
print(dados_filtrados_two)
cat("print não funciona com tibble\n")

cat("aqui estão os dados mas em JSON\n")

dados_filtrados_json_two <- 
  toJSON(dados_filtrados_two, pretty = TRUE)
cat(dados_filtrados_json_two)
...
ACHO QUE SERÁ ÚTIL (falta ajustes ou curadoria)
...
cat("Estes são os dados raspados:\n")
print(dados_filtrados)

cat("Não está aparecendo nada? ou está na tabela?\n")
...
ACHO QUE NÃO SERÁ ÚTIL (Apenas salvando)
