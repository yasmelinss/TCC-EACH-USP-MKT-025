...
walk(
  ...
  ~ {
    ...
    # pega o intervalo de dias desse período
    ...
    set.seed(123)  # layout reprodutível
    
    # 1) layout com o grafo COMPLETO
    layout_full <- create_layout(g_full, layout = "fr")
    
    # 2) FILTRA o layout: só nós com grau_in >= limiar_in
    layout_filt <- 
      layout_full |>
      dplyr::filter(!is.na(grau_in), grau_in >= limiar_in)
    
    # se quiser inspecionar quantos nós sobraram:
    # nrow(layout_filt)
    ...
    p <-
      ...
      ggtitle(paste0(
        "Rede de likes – ", periodo_nome,
        "\n(", data_lab, ")",
        "\nNós com grau_in \u2265 ", limiar_in
      ))
...
walk(
  ...
  ~ {
    ...
    # 🔸 1) filtra nós no GRAFO (não no layout)
    g_filt <- 
      g_full |>
      activate(nodes) |>
...
metricas_p5 <- grafos_periodos$periodo5 |> activate(nodes) |> as_tibble()
metricas_p5 |> arrange(desc(grau_out)) |> head(20) 
  
saveRDS(metricas_p5, "Dados/metricas_periodo5.rds")
