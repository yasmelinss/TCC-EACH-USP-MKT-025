...
matriz.likes.20240907.saindo <- 
  ...
  left_join(all.actors.likes, by = c("actor.influencer.num" = "num")) |> 
  rename(
    actor.influencer = did,
...
matriz.likes.20240907.entrando <- 
  ...
  summarise(quantos = sum(quantos)) |> 
  left_join(all.actors.likes, by = c("actor.influenced.num" = "num")) |> 
  rename(
    actor.influenced = did,
...
matriz.likes.20240907.saldo <- 
  full_join(
    matriz.likes.20240907.saindo,
    matriz.likes.20240907.entrando,
    by = c("actor.influencer.num" = "actor.influenced.num"),
    keep = TRUE
  ) |> 
  mutate(
    across(
  ...
  rename(
    actor = actor.influencer,
    actor.num = actor.influencer.num
  ) |> 
  select(
    actor, 
    actor.num, 
